<script>
  ...
  async function createBuildAndPersistQueue() {
    ...
    try {
      ...
      // Insert manufactured parts into parts table
      const createdPartIds = [];
      for (const it of queue.manufactured) {
        const wf = it.workflow || 'mill';
        const file_format =
          wf === '3d-print' ? 'stl' :
        ...
        const baseInsert = {
          ...
          file_name: '',
          // include router meta in file_url if present
          file_url: it.router_meta ? JSON.stringify({ router_meta: it.router_meta }) : ''
        };

        // Try with Onshape fields and stock_assignment if column exists
        let partRow = null;
        ...
        try {
          const withOnshape = {
            ...
            onshape_document_id: it.onshape_document_id,
            onshape_wvm: it.onshape_wvm,
            onshape_wvmid: it.onshape_wvmid,
            onshape_element_id: it.onshape_element_id,
            onshape_part_id: it.onshape_part_id,
            file_format,
        ...
        // attach drawing EID for lathe/mill (ignore missing column)
        try {
          if (partRow && it.drawingEID && (wf === 'lathe' || wf === 'mill')) {
            const { error: drawUpdateError } = await supabase
              .from('parts')
              .update({ onshape_drawing_element_id: it.drawingEID })
              .eq('id', partRow.id);
            if (drawUpdateError) {
              console.warn('Could not store drawing element ID (column may not exist):', drawUpdateError?.message || drawUpdateError);
            }
          }
        } catch (e) {
        ...
        if (partRow?.id) createdPartIds.push(partRow.id);
      }

      // Insert COTS into purchasing
      const createdPurchasingIds = [];
      for (const it of queue.cots) {
        ...
        const { data, error } = await supabase.from('purchasing').insert([purchasingInsertData]).select();
        if (error) throw error;
