...
//   comp_level=qm (default: qm) – filter by comp level
...
export async function GET({ url }) {
  ...
  const compLevel = url.searchParams.get('comp_level') || 'qm';
  ...
  try {
    ...
    if (compLevel) data = (data || []).filter((m) => m.comp_level === compLevel);
    data = data.sort((a, b) => (a.match_number || 0) - (b.match_number || 0));
