...
> **This guide covers the original, unmodified upstream Runner talking to Valor 6800's own AutoCAM WebUI (path 1 in the "How this relates" section below) - useful for evaluation, or for diffing against the real upstream behavior.** Spartans Hub has since built **path 2**: a native **Fusion CAM** section (`/autocam/fusion`) backed by this app's own `cam_jobs` table, paired with a forked Runner at [`autocam/fusion/runner/`](autocam/fusion/runner/) that polls this app's own `/api/fusion-runner` endpoint instead of Valor 6800's WebUI. **If you're setting up a real production Fusion 360 machine for Spartan Robotics, follow [`autocam/fusion/runner/README.md`](autocam/fusion/runner/README.md) instead of this file** - it covers the same `.overridepath`/`requests` gotcha (Step 4 below) plus the actual `.env` values our fork expects. This file is kept for historical/evaluation reference and because the fork's Fusion-side mechanics (templates, `.overridepath`, add-in installation) are otherwise identical to what's documented here.
...
**Two real integration paths were considered here - path 2 is the one Spartans Hub actually built:**
...
2. **Fork the Runner and point its networking layer at this app's own backend** instead of the AutoCAM WebUI - reuse the genuinely valuable, hard-to-build part (the Fusion Python automation: template application, toolpath generation, G-code export, status reporting) while keeping one single source of truth for jobs (this app's existing `cam_jobs`/`cam_machines` schema, `claimed_by`/`claimed_at` job-claim lifecycle already designed for this).

**Path 2 is what's live today.** Spartans Hub has a native **Fusion CAM** section (`src/routes/autocam/fusion/`, reachable from `/autocam`) backed by new `fusion_parts`/`fusion_plates`/`fusion_box_tubes`/`fusion_part_categories` tables plus the reused `cam_jobs`/`cam_machines`/`cam_tools`/`cam_materials` tables, and a claim/complete/fail endpoint at `src/routes/api/fusion-runner/+server.js` (the CAS lifecycle this section originally sketched as `GET /api/cam-jobs/claim` / `POST /api/cam-jobs/:id/complete` - built as a single action-routed endpoint instead). The forked Runner lives at [`autocam/fusion/runner/`](autocam/fusion/runner/) (its own [README](autocam/fusion/runner/README.md) has the real setup steps); an unmodified copy of both the original WebUI and Runner is vendored at `autocam/fusion/_upstream/` and `autocam/fusion/runner/_upstream/` for reference/diffing. No Teams/Invites/API-key-per-team layer was ported - the Runner authenticates with a single shared-secret bearer token (`FUSION_RUNNER_TOKEN`), and human users go through this app's own Supabase Auth + `canManageCamProfiles` permission, matching Spartans Hub's existing single-org model instead of the original's multi-tenant one.

Given plate/tube CAM (the Runner's actual current scope) is already covered by `autocam/routing.js` without any of this, **the strongest case for adopting the Runner at all is specifically for milling** - which is exactly the gap Fusion CAM now fills. Building real per-machine CAM templates for Spartan Robotics' equipment (Step 7 above) is still the actual hard part and remains unstarted/ungated by any of this software work.
