...
```bash
cp -R autocam/fusion/runner "$HOME/Library/Application Support/Autodesk/Autodesk Fusion 360/API/AddIns/test"
```

**The destination folder must be named `test`, not `runner`.** Fusion requires the AddIns folder name, the main `.py` file name, and the `.manifest` file name to all match exactly — the files inside are `test.py`/`test.manifest` (that's the real, current state of this repo, not a typo), so the folder has to be `test` too, or Fusion won't even list it as an add-in. This is the single most common thing to get stuck on.

(Windows path: `%APPDATA%\Autodesk\Autodesk Fusion 360\API\AddIns\test`)
...
echo ~/fusion-runner-deps > "$HOME/Library/Application Support/Autodesk/Autodesk Fusion 360/API/AddIns/test/.overridepath"
```
...
```bash
cd "$HOME/Library/Application Support/Autodesk/Autodesk Fusion 360/API/AddIns/test"
cp .env.example .env
```
...
3. **Add-Ins** tab inside that dialog (not "My Scripts") → find **test** → select it → **Run**.
4. It'll be listed as **test**, not "Fusion CAM" — expected, matches the internal file names from step 1.
