...
def start(data, session):
    ...
    try:
        ...
        # TODO(fusion-cam-port): the arrangement screenshot (screenshot_path)
        # isn't uploaded anywhere - /api/fusion-runner's action=complete only
        # accepts gcode/gcodeFileName/stats fields (see
        # src/routes/api/fusion-runner/+server.js), and this Runner is
        # deliberately scoped to that one bearer-token-authenticated
        # endpoint rather than holding direct Supabase Storage credentials
        # (a wider trust boundary than a single job-lifecycle API needs).
        # Uploading the preview image would need a real new upload path,
        # designed and tested against a live job queue - flagged here rather
        # than guessed at, same as setupTemp.py's file-download gap noted in
        # test.py's handleServer(). plate:arrange jobs carry no G-code
        # either way, so "gcode" below is always empty - this job kind's
        # actual result (fit vs. excess/oversized) lives in "stats".
        plate_id = str(data.get("payload", {}).get("plate_id", "unknown"))
        if not has_excess and not has_oversized:
            ...
            # Mirrors upstream: an arrangement that didn't fully fit still
            # "completes" (the job ran; the result is "here's what didn't
            # fit"), it isn't a job failure - the error text and part lists
            # travel in stats, not as a send_job_error() call.
            resp = session.post(
                f"{BASE_URL}/api/fusion-runner",
                params={"action": "complete"},
                json={
        ...
        context_label = f"Plate {plate_id} job {job_id} completion upload"
        ensure_completion_response(session, resp, job_id, context_label)
    except Exception:
        if app:
