...
CREATE TABLE IF NOT EXISTS public.cam_machines (
  ...
  CONSTRAINT cam_machines_operation_type_check CHECK (operation_type = ANY (ARRAY['turning'::text, 'routing'::text, 'milling'::text])),
  CONSTRAINT cam_machines_default_material_id_fkey FOREIGN KEY (default_material_id) REFERENCES public.cam_materials(id) ON DELETE SET NULL,
  CONSTRAINT cam_machines_default_tool_id_fkey FOREIGN KEY (default_tool_id) REFERENCES public.cam_tools(id) ON DELETE SET NULL,
  CONSTRAINT cam_machines_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id) ON DELETE SET NULL
);
...
CREATE TABLE IF NOT EXISTS public.cam_jobs (
  ...
  name text, -- user-given job name; falls back to the part/file name in the UI when blank
  source_type text NOT NULL DEFAULT 'upload', -- 'upload' or 'part'
  part_id bigint, -- set when source_type = 'part'
  step_file_name text, -- storage path of the STEP file - both the 3D viewer AND CAM generation read this

  operation_type text NOT NULL DEFAULT 'routing', -- 'turning' | 'routing' | 'milling' (milling has no generator yet)
  params jsonb NOT NULL DEFAULT '{}'::jsonb, -- operation-specific parameters, see src/lib/cam/*.js
  ...
  claimed_by text, -- reserved for a future external Runner (milling only - see autocam-runner/README.md)
  claimed_at timestamp with time zone,

  gcode text,
  gcode_file_name text,
  gcode_format text NOT NULL DEFAULT 'ngc', -- must stay 'ngc'
  ...
  CONSTRAINT cam_jobs_source_type_check CHECK (source_type = ANY (ARRAY['upload'::text, 'part'::text])),
  CONSTRAINT cam_jobs_operation_type_check CHECK (operation_type = ANY (ARRAY['turning'::text, 'routing'::text, 'milling'::text])),
  CONSTRAINT cam_jobs_status_check CHECK (status = ANY (ARRAY['queued'::text, 'claimed'::text, 'processing'::text, 'completed'::text, 'failed'::text, 'rejected'::text])),
  CONSTRAINT cam_jobs_part_id_fkey FOREIGN KEY (part_id) REFERENCES public.parts(id) ON DELETE SET NULL,
  CONSTRAINT cam_jobs_material_id_fkey FOREIGN KEY (material_id) REFERENCES public.cam_materials(id) ON DELETE SET NULL,
  CONSTRAINT cam_jobs_tool_id_fkey FOREIGN KEY (tool_id) REFERENCES public.cam_tools(id) ON DELETE SET NULL,
  CONSTRAINT cam_jobs_machine_id_fkey FOREIGN KEY (machine_id) REFERENCES public.cam_machines(id) ON DELETE SET NULL,
  CONSTRAINT cam_jobs_requested_by_fkey FOREIGN KEY (requested_by) REFERENCES auth.users(id) ON DELETE SET NULL
);
